...
> **R:**Como se explico anteriormente el preprocesador copia y pega el contenido de otros archivos dentro del programa lo cual aumenta el numero de lineas
...
> **R:**porque en el preprocesamiento sus acciones principales son eliminar comentarios reemplazandolos por un espacio en blanco por lo cual al hacer el grep no encuentra nada
...
> **R:**indidca el nuemro de linea y el archivo original de donde proviene el codigo que sigue , proviene del stdio.h ->c:\mingw\include\stdio.h
...
> **R:**porque en el archivo programa.o se usa pero no esta definida mientras que el alrchivo matematica.o significa que esta definida la funcion
...
> **R:**quedan porque son bibliotecas dinamicas,lo resuelve el cargador dinámico (dyld/ld.so) las resuelva cuando el programa se ejecute.
...
> **R:**CUADRADO(x) es solo un reemplazo de texto lo cual desaparece en el preprocesamiento, mientras que sumar(a, b) se compila y se enlaza despues y tambien tiene identificador de tipos
...
> **R:**T esta definido en la sección text (código ejecutable), funciones.Mientras que Desta definido en la sección data, variable global con valor inicial (ej: int x = 5;)
...
> **R:**Porque gcc ya linkea libc automaticamente por eso no hay que agregarla
